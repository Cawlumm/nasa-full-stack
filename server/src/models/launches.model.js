...
async function populateLaunches() {
  ...
  if(response.status !== 200) {
    console.error(`Error downloading launch data from SpaceX API (${response})`);
...
async function loadLaunchData() {
  ...
  if (firstLaunch) {
    console.log("Launch data already laoded");
  } else {
...
async function getAllLaunches(skip, limit) {
  return await launchesDatebase.find({}, { _id: 0, __v: 0 })
  .sort({
    "flightNumber": 1
  })
  .skip(skip)
